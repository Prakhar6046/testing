...
export function HeaderClient({ data }: HeaderClientProps) {
  ...
  return (
    <>
      ...
      <header
        className={`fixed top-0 left-0 right-0 z-50 p-8 transition-all duration-300 ease-in-out ${
          isScrolled ? 'bg-transparent' : 'bg-background'
        }`}
      >
        <div
          className={`container mx-auto bg-white flex justify-between items-center relative z-50 md:px-5 px-4 py-3 rounded-[100px] transition-all duration-300 ${
            isScrolled ? 'border-2 border-secondary' : 'border-2 border-transparent'
          }`}
        >
          {/* Logo */}
          <div className="flex items-center">
            <Link href="/" className="flex justify-center items-center w-[128px] h-[52px]">
              <Image
                src={logoUrl}
                alt={logoAlt}
                width={128}
