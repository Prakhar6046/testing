          ...
          <div className={`grid grid-cols-1 ${columnStyles[layout.columns]} gap-5 mb-12`}>
            {caseStudiesData.map((caseStudy, index) => {
              ...
              return (
                <div
                  key={index}
                  className={`${cardRadiusStyles[style.cardRadius]} bg-white cursor-pointer ${cardStyle.container} ${
                ...
                >
                  ...
                  <div className={`${cardStyle.padding} flex flex-col flex-grow`}>
                    ...
                    {(caseStudy.category || caseStudy.tag) && (
                      ...
                      >
                        {caseStudy.category ? caseStudy.category.replace('-', ' ').toUpperCase() : caseStudy.tag}
                      </div>
                    )}
                    ...
                    {isClickable ? (
                      <Link
                        href={caseStudyLink}
                        className="md:text-2xl text-xl font-medium text-black mb-2 hover:text-wils_orang hover:underline transition-colors"
                        {...(caseStudy.linkType === 'external'
                          ? { target: '_blank', rel: 'noopener noreferrer' }
                          : {})}
                      >
                      ...
                      <h3 className="md:text-2xl text-xl font-medium text-black mb-2">
                        {caseStudy.title}
                      </h3>
                    )}
